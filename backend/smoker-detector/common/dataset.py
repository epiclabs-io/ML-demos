...
def load_image(filename, desired_shape=(224,224)):
    raw = misc.imread(filename)
    shape = np.asarray(raw).shape
    if shape[0] != desired_shape[0] or shape[1] != desired_shape[1]:
        raw = reshape_image_adding_bars(raw, shape, desired_shape, print_log=False)
        basename = os.path.basename(filename)
        misc.imsave("tmp/" + basename+"_res.jpg", raw)
    image = np.asarray(raw, dtype=np.float32) / 255
    image = np.reshape(image, -1)
    return image


def reshape_image_adding_bars(raw, shape, target_shape=(180, 240), print_log=True):
